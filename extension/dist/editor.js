...
if (editor) {
    editor.on("scene:load", function () {
        window.setTimeout(function () {
            ...
            var item = editor.call("assets:findOne", function (asset) {
                return (asset.get("type") === "script" &&
                    asset.get("name") === "uranus-editor-sdk.js");
            });
            if (!item)
                return;
            var asset = app.assets.get(item[1].get("id"));
            if (asset) {
                // --- if the loader is included, we load its contents and execute them
                var localUrl = asset.getFileUrl();
                fetch(localUrl).then(function (response) { return __awaiter(_this, void 0, void 0, function () {
                    var code, script;
